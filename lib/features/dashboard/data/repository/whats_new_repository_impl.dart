...
import '../../domain/repository/whats_new_repository.dart';
import '../model/whats_new.dart';
...
class WhatsNewRepositoryImpl implements WhatsNewRepository {
  ...
  @override
  Future<DataState<List<WhatsNew>>> getResponse(formData, serviceId) async {
    try {
      ...
      if (httpResponse.response.statusCode == HttpStatus.ok) {
        ...
        if (responseStatus.isNotEmpty) {
          List<WhatsNew> resultSet =
              EncryptedResponse.fromJson(clientJsonResponse[0])
                  .resultSet!
                  .map<WhatsNew>((dynamic i) =>
                      WhatsNew.fromMap(i as Map<String, dynamic>))
                  .toList();
